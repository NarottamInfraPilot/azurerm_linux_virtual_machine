...
module "resource_group" {
  ...
  prevent_destroy    = true  # Prevent accidental destruction in UAT
}
...
module "virtual_network" {
  ...
  address_space       = ["10.1.0.0/16"]  # Different CIDR for UAT
...
module "linux_vms" {
  ...
  enable_backup   = lookup(each.value, "enable_backup", true)  # Enable backup by default in UAT
